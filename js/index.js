...
var ImageFile, ImageFileMimicLogic, changeCond2, checkMimic, clear, compareIndexes, cond2cond1, debug, downDementionStat, genPattern, getCellNum, getColorIndexes, getColorNearIndexes, getConfirmTypes, getLineIndexes, getMimicIndexes, getMimicNearIndexes, getNearIndexes, getNumCommodity, getNumEquip, getNumMad, getNumMimicMax, getNumMimicMin, getNumMoney, getPatternMad, getTd, getValidPatterns, getX, getY, init, isContainColorNearMimic, isContainType, isContainTypeCount, isItemDetail, isMimicNearly, isNotContainType, isValidPattern, judge, mustConsiderType, onPasteImage, parseImage, reset, statValidPatterns, viewDetail, viewTitle;
...
ImageFile = class ImageFile {
  ...
  binarize(x = 0, y = 0, w = this.getWidth(), h = this.getHeight(), r = null, g = null, b = null) {
    var bCond, gCond, j, l, rCond, ref, ref1, rgb;
    ...
    for (x = j = 0, ref = rgb.length; (0 <= ref ? j < ref : j > ref); x = 0 <= ref ? ++j : --j) {
      for (y = l = 0, ref1 = rgb[x].length; (0 <= ref1 ? l < ref1 : l > ref1); y = 0 <= ref1 ? ++l : --l) {
        rCond = r === null || rgb[x][y].r >= r;
        gCond = g === null || rgb[x][y].g >= g;
        bCond = b === null || rgb[x][y].b >= b;
        rgb[x][y] = rCond && gCond && bCond;
...
parseImage = function(base64) {
  ...
  callback = function() {
    ...
    return console.log(img.getLeftUpPoint());
  };
...
judge = function() {
  var allowed, cellNum, colors, conds, condsIndex, condsLight, confirms, dummy, i1, index, j, kakuteiTypes, key, l, len, len1, len2, len3, madPattern, madPatternOnlyMimicResearch, numCommodity, numEquip, numMad, numMimic, numMimicMax, numMimicMin, numMoney, nums, o, patterns, q, ref, ref1, ref2, ref3, ref4, ref5, ref6, s, stockedIndexes, t, typeConst, u, val, validOnlyMimicResearch, validPattern, validPatternOnlyMimicResearch, valids, validsOnlyMimicResearch, value, x, y, z;
  viewTitle(null);
  x = getX();
  y = getY();
  cellNum = getCellNum();
...
// 条件を緩める
  ...
  valids = [];
  for (s = 0, len2 = validsOnlyMimicResearch.length; s < len2; s++) {
    validOnlyMimicResearch = validsOnlyMimicResearch[s];
    numMimic = 0;
    for (t = 0, len3 = validOnlyMimicResearch.length; t < len3; t++) {
      typeConst = validOnlyMimicResearch[t];
...
// アイテム詳細の指定がない場合は全パターン
      for (numEquip = u = 0, ref4 = cellNum - numMimic; (0 <= ref4 ? u <= ref4 : u >= ref4); numEquip = 0 <= ref4 ? ++u : --u) {
        if (numEquip < kakuteiTypes[window.CONSTS.EQUIP]) {
          continue;
        }
        for (numMoney = z = 0, ref5 = cellNum - numMimic; (0 <= ref5 ? z <= ref5 : z >= ref5); numMoney = 0 <= ref5 ? ++z : --z) {
          if (numMoney < kakuteiTypes[window.CONSTS.MONEY]) {
            continue;
          }
          for (numCommodity = i1 = 0, ref6 = cellNum - numMimic; (0 <= ref6 ? i1 <= ref6 : i1 >= ref6); numCommodity = 0 <= ref6 ? ++i1 : --i1) {
...
ImageFileMimicLogic = class ImageFileMimicLogic extends ImageFile {
  // 左上の座標を取得
  getLeftUpPoint() {
    var COUNT_MIN, RGB_MIN, binary, count, isWhite, j, l, ref, ref1, res, x, xStart, y;
    RGB_MIN = {
      R: 200,
      G: 200,
      B: 200
    ...
    COUNT_MIN = 20;
    binary = this.binarize(0, 0, void 0, void 0, RGB_MIN.R, RGB_MIN.G, RGB_MIN.B);
    console.log(binary);
    res = [];
