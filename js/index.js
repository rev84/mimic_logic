...
parseImage = function(base64) {
  ...
  callback = function() {
    var canvas, condIndex, ctx, h, htmls, imageFile, j, leftups, len, matchRates, rate, ref, results, targetImageFile, w, x, y;
    if (!img.isLoaded()) {
    ...
    for (j = 0, len = leftups.length; j < len; j++) {
      ...
      htmls = [];
      for (condIndex in matchRates) {
        rate = matchRates[condIndex];
        htmls.push('' + condIndex + ':' + rate);
      }
      results.push($('#debug').append($('<p>').html(htmls.join('<br>'))));
    }
    return results;
  };
