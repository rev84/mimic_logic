...
judge = function() {
  var cellNum, colors, conds, condsIndex, condsLight, j, l, len, len1, numCommodity, numEquip, numMimic, numMimicMax, numMimicMin, numMoney, nums, o, patterns, q, r, ref, ref1, ref2, ref3, ref4, ref5, s, stockedIndexes, t, typeConst, validOnlyMimicResearch, validPattern, validPatternOnlyMimicResearch, valids, validsOnlyMimicResearch, x, y;
  x = getX();
  y = getY();
  cellNum = getCellNum();
...
// 条件を緩める
  ...
  patterns = [];
  for (numMimic = l = ref1 = numMimicMin, ref2 = numMimicMax; (ref1 <= ref2 ? l <= ref2 : l >= ref2); numMimic = ref1 <= ref2 ? ++l : --l) {
  ...
  // 確定パターンがないなら全リセット
  valids = [];
  for (o = 0, len = validsOnlyMimicResearch.length; o < len; o++) {
    validOnlyMimicResearch = validsOnlyMimicResearch[o];
    numMimic = 0;
    for (q = 0, len1 = validOnlyMimicResearch.length; q < len1; q++) {
      typeConst = validOnlyMimicResearch[q];
      if (typeConst === window.CONSTS.MIMIC) {
        numMimic++;
      }
    }
    for (numEquip = r = 0, ref3 = cellNum - numMimic; (0 <= ref3 ? r <= ref3 : r >= ref3); numEquip = 0 <= ref3 ? ++r : --r) {
      for (numMoney = s = 0, ref4 = cellNum - numMimic; (0 <= ref4 ? s <= ref4 : s >= ref4); numMoney = 0 <= ref4 ? ++s : --s) {
        for (numCommodity = t = 0, ref5 = cellNum - numMimic; (0 <= ref5 ? t <= ref5 : t >= ref5); numCommodity = 0 <= ref5 ? ++t : --t) {
    ...
    for (index = l = 0, len1 = remain.length; l < len1; index = ++l) {
      ...
      if (pattern !== null) {
        switch (pattern[m.length]) {
          case window.CONSTS.NOT_MIMIC:
            if (consts[index] === window.CONSTS.MIMIC) {
              continue;
            }
            break;
          case window.CONSTS.MIMIC:
